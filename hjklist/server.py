...
from data import *
from utils import *
import stripe; stripe.api_key = config.stripe_secret
...
def do_signup(**context):
    ...
    stripe_token = bottle.request.forms.get('stripe_token')
    if bottle.request.forms:
        ...
        # Create their stripe association
        stripe_customer = stripe.Customer.create(
            card = stripe_token,
            plan = 'hjklist_beta_5mo',
            email = email
        )
        # Set up new user
...
update_payment_form = renderer('update_payment_form')
...
def update_payment(**context):
    session = context['session']
    stripe_token = bottle.request.forms.get('stripe_token')
    stripe_customer = stripe.Customer.retrieve(session.user.stripe_id)
    stripe_customer.card = stripe_token
    stripe_customer.save()
    return message("Your payment details have been updated.", **context)
...
route_map = {
    ...
    '#/account/password/reset': reset_password,
    '/account/payment': [authenticate, update_payment_form],
    '#/account/payment': [authenticate, update_payment],
